...
# Secure by default: the runbook says to copy .env.example verbatim, so the
# example must not ship an insecure value — an operator missing an edit meant
# a 30-day session token going out over plaintext on any pre-HSTS first
# visit. Dev opts out explicitly (.env.example carries the line commented
# out), which is the direction that should require an action.
